...
describe('coordinate parser', () => {
  ...
  it('recognizes C as absence and VAP as holiday without review warnings', () => { const r = parsePdfItems([page([[['01/10/2026',52],['C',131]], [['02/10/2026',52],['VAP',131]]])], 'synthetic.pdf'); expect(r.issues.some(i => i.code === 'STATUS')).toBe(false); expect(r.schedule!.days.map(d => d.status)).toEqual(['absence','holiday']); });
